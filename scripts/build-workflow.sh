...
#
# Steps:
#   1. Build cmd/password-generator-alfred as a universal (amd64+arm64)
...
GOOS=darwin GOARCH=amd64 go build -o "${BUILD_DIR}/password-generator-alfred-amd64" ./cmd/password-generator-alfred
GOOS=darwin GOARCH=arm64 go build -o "${BUILD_DIR}/password-generator-alfred-arm64" ./cmd/password-generator-alfred
lipo -create -output "${BUILD_DIR}/password-generator-alfred" \
  "${BUILD_DIR}/password-generator-alfred-amd64" \
  "${BUILD_DIR}/password-generator-alfred-arm64"
rm "${BUILD_DIR}/password-generator-alfred-amd64" "${BUILD_DIR}/password-generator-alfred-arm64"
chmod +x "${BUILD_DIR}/password-generator-alfred"
lipo -info "${BUILD_DIR}/password-generator-alfred"

if [ -n "${CODESIGN_IDENTITY:-}" ]; then
  echo "→ Signing entrypoint binary (${CODESIGN_IDENTITY})"
  codesign --force --options runtime --timestamp --sign "$CODESIGN_IDENTITY" \
    "${BUILD_DIR}/password-generator-alfred"
  codesign --verify --strict --verbose=2 "${BUILD_DIR}/password-generator-alfred"

  if [ -n "${NOTARY_KEY_ID:-}" ]; then
    "${REPO_ROOT}/scripts/notarize-binary.sh" "${BUILD_DIR}/password-generator-alfred"
  fi
else

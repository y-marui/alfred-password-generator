...
Alfred 5 Script Filter ワークフロー。長さ・文字セットを指定してパスワードを生成する。
Go（サードパーティ依存なし）、`cmd/`+`internal/` レイアウト、CI/CD 完備。
対象: 個人〜3人規模の開発チーム。ライセンス: MIT。

```
cmd/password-generator-alfred/  ← Alfred が実行する唯一のバイナリ
internal/passgen/               ← パスワード生成ロジック（コア、stdlib のみ）
internal/passgencmd/            ← コマンドディスパッチ・引数解析・help
...
- `cmd/password-generator-alfred/main.go` は Alfred が実行する**唯一のバイナリ**。ビジネスロジックを書かない
- `internal/passgencmd/` はディスパッチ・引数解析のみ — 生成ロジックは `internal/passgen/` に置く
- `internal/passgen/` は Alfred 非依存の純粋ロジック — stdlib のみ使用し、単体でテスト可能に保つ
- すべての応答は `internal/scriptfilter.Response.Write()` を経由する
- `main()` は panic を `recover()` でラップする（未捕捉 panic = Alfred が空白表示になる）

### Testing Conventions

- `internal/passgen/`・`internal/passgencmd/` をテスト対象とする（`go test ./...`）
- 外部 I/O は行わない（`internal/passgen` は `crypto/rand` 以外 stdlib のみ使用）
- Alfred 環境変数への依存はない（`history` は Go バイナリ自身が読むことはなく、Alfred の
  ネイティブ Conditional/Clipboard ノードのみが参照する）
...
- `cmd/password-generator-alfred/main.go` へのビジネスロジックの追加
- テストでの実際の HTTP 通信
